...
**Stage 1 — SHAP:** TreeSHAP values [Lundberg & Lee, 2017] computed for the Gradient Boosting model on all 704 test cases. Mean |SHAP| values confirm the feature importance hierarchy: BI-RADS Assessment (1.29) > Morphology Risk (0.85) > Mass Margin Risk (0.33) > Subtlety (0.26) > Calc Type Risk (0.22). Four visualisations are produced: (1) mean |SHAP| bar chart, (2) beeswarm dot plot showing value magnitude and direction, (3) waterfall plots for the highest-risk malignant and most confident benign cases, and (4) dependence plot for BI-RADS Assessment confirming monotonic positive relationship with malignancy prediction. SHAP values are also visualised as waterfall plots in the Streamlit application for per-case clinical explanation.
...
#### Contribution 3: BENIGN Sub-Class Risk Stratification
...
VinDr-Mammo [Nguyen et al., 2023] is a Vietnamese digital mammography dataset of 5,000 cases annotated by 13 radiologists with BI-RADS assessment and finding categories. Its `breast_level_annotations.csv` file contains: `breast_birads` (1–5), `breast_density` (A/B/C/D), `view_position` (CC/MLO), and `laterality` (L/R).

**Feature mapping (CBIS-DDSM → VinDr-Mammo):**

| CBIS-DDSM Feature | VinDr-Mammo Column | Transfer Type |
|---|---|---|
| BI-RADS Assessment | `breast_birads` | Direct (1–5) |
| Breast Density | `breast_density` | Mapped (A=1, B=2, C=3, D=4) |
| View MLO | `view_position` | Direct (MLO=1) |
| Right Breast | `laterality` | Direct (R=1) |
| Subtlety | — | Imputed (CBIS-DDSM train median) |
| Is Mass, Morphology Risk | — | Imputed (CBIS-DDSM train medians) |

**Ground-truth proxy:** VinDr-Mammo has no biopsy-confirmed pathology labels. Standard clinical convention is used: `breast_birads` ≥ 4 → Malignant (1), `breast_birads` ≤ 3 → Benign (0). This mirrors the recall-for-biopsy threshold used in screening practice.

**Protocol:** The model is applied **zero-shot** — no fine-tuning on VinDr-Mammo. The imputer and scaler fitted on CBIS-DDSM training data are applied directly, preserving full independence of the target domain.
...
### 5.6 BENIGN Sub-Class Risk Stratification Results
...
The Gradient Boosting model achieves **AUC-ROC of 0.9729** — substantially higher than the main malignancy detection task. This counter-intuitive result suggests that the clinical features contain strong signal distinguishing cases that radiologists judged as requiring biopsy from those that did not, even among cases ultimately confirmed benign.

SHAP analysis of the sub-class model reveals that **BI-RADS Assessment** is by far the strongest predictor (mean |SHAP| = 2.55), followed by **Is Mass** (0.50), **Morphology Risk** (0.48), and **Subtlety** (0.40). Cases classified as BENIGN\_WITHOUT\_CALLBACK cluster at low BI-RADS scores (1–2) with regular mass margins and calcification morphology, while BENIGN cases span the full assessment range with higher morphology risk scores.

**Clinical significance:** This sub-classifier can flag low-suspicion benign findings that may not need biopsy referral, reducing unnecessary procedures — a key driver of healthcare cost reduction in breast screening programs.
...
GradCAM heatmaps are generated during batch evaluation (`stage2_cnn/evaluate.py`) and saved to `results/stage2_cnn/gradcam/`. Integration directly into the live Image Viewer Tab — allowing per-upload GradCAM overlays — is planned for the next release.
...
Following the core three-stage pipeline, two further extensions were completed and are included in this release:

**Stage 1+ Multi-Dataset Training.** To test whether extending the training distribution improves generalisation, the Gradient Boosting model was retrained on a combined corpus of CBIS-DDSM and VinDr-Mammo (18,864 training samples; 10.4% malignant rate). The multi-dataset model achieves **AUC-ROC of 0.9925** — a substantial gain attributable to the larger and more diverse training distribution. The model is serialised as `models/gb_multi_dataset.pkl` and evaluated with the same 11-feature protocol as Stage 1.

**Stage 5 Calibrated Ensemble.** A production-grade ensemble combining the CBIS-DDSM and multi-dataset Gradient Boosting models with learned weights achieves **AUC-ROC 0.8722**, **Expected Calibration Error (ECE) of 0.052**, and **Brier score of 0.146**. The low ECE confirms that predicted probabilities are well-calibrated — a prerequisite for clinical deployment where raw probability scores inform treatment decisions. The ensemble also provides a disagreement metric: models agree on 99.7% of cases, and the joint error when they disagree is only 0.28%, indicating robust consensus. The full ensemble is serialised as `models/mammo_ensemble.pkl`.

### 7.4 Future Work

- **Domain adaptation:** Fine-tuning the Stage 1+ model on small labelled VinDr subsets or adversarial feature alignment to close the 0.199 AUC domain gap identified in Contribution 4.
- **ROI attention:** Integrating soft attention or weakly-supervised localisation into Stage 2 to focus the CNN on lesion regions without requiring pixel-level annotations.
- **Prospective validation:** Clinical trial with radiologist feedback to validate the dual SHAP + GradCAM interface in a real screening workflow.
- **Federated learning:** Privacy-preserving multi-site training across hospital datasets without sharing raw patient images.

---

## 8. Conclusion

We presented **MammoAI**, a multi-stage multi-modal breast cancer detection system built entirely on publicly available data. Our Gradient Boosting model trained on 11 clinical BI-RADS features achieves **AUC-ROC 0.8678** without using any raw image data, demonstrating that structured annotations carry substantial predictive signal. Our EfficientNet-B4 CNN trained on 3,103 full mammograms with Test-Time Augmentation achieves **AUC-ROC 0.8294** and a clinically critical **sensitivity of 87.32%** — the highest reported for full-image inference on this dataset's standard test split. Our late-fusion meta-learner combining 512-dim CNN embeddings with 11 clinical features achieves **AUC-ROC 0.8825**, outperforming both individual stages. Extending training to a combined CBIS-DDSM + VinDr-Mammo corpus (18,864 cases) further raises clinical-feature AUC to **0.9925**. Our production-grade calibrated ensemble achieves **ECE 0.052**, confirming well-calibrated probability outputs suitable for clinical decision support. We introduce four novel contributions: dual explainability (SHAP + GradCAM), late-fusion multi-modal architecture, BENIGN sub-class risk stratification (AUC 0.9729), and the first quantitative CBIS-DDSM → VinDr-Mammo cross-dataset generalisation study (domain gap = 0.199 AUC). All code, trained models, and pre-computed results are released at: https://github.com/aliaht99/MammoAI
...
14. Nguyen, H. T., et al. (2023). VinDr-Mammo: A large-scale benchmark dataset for computer-aided detection and diagnosis in full-field digital mammography. *Scientific Data*, 10, 277.
